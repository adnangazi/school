...
/********************************************************************************************************************************************************************
	...
	G riceve come parametri formali i parametri attuali alla chiamata da F, ovvero:
		- S + 1: array di caratteri di lunghezza >= 0, in quanto la chiamata ricorsiva avviene sse la lunghezza si S > 3 ==> S + 1
		è un array di dimensione positiva
		- n - 2: intero >= 0, in quanto la chiamata ricorsiva avviene sse n > 3 ==> n - 2 >= 0
	...
	==> presumendo la corretteza del caso ricorsivo in G (che avviene sse n > 3), abbiamo che G rispetta il POST-condizione della funzione.

	Usando la correttezza di G, dimostro la correttezza di F:
		In F, se la lunghezza della stringa è <= 3, si passa direttamente ai casi base:
		- caso base 1: return true sse il primo e l'ultimo elemento sono uguali:
			...
			- se la striga ha dimensione 3, la stringa è palindroma se il primo elemento coincide con l'ultimo elemento ==> l'unico elemento
			in mezzo è palindromo perchè di dimensione 1.
		- caso base 2: return false sse il caso base 1 non è verificato ==> stringa di dimensione 2 0 3 con primo e ultimo elemento diversi.
