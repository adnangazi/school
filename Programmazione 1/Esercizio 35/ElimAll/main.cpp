...
nodo * elimAll(nodo * L, int x) { //restituito nuova lista modificata: termina appena trova nodo con info == x
    if (L) {
        if (L->info != x) {
            return new nodo (L->info, elimAll(L->next, x)); //caso ricorsivo: restituita nuova lista creata con nodi con info != x
        } else {
            nodo * temp = L->next;
            delete L;
            return elimAll(temp, x); //caso base 1: restituito resto della lista ben formata
        }
    }

    return 0; //caso base 2
}
//POST=(restituisce una lista che contiene tutti i nodi di Lista(L) a parte ogni con info = x, che, se esiste, va anche deallocato)

//PRE=(Lista(L) ben formata e non vuota e col primo nodo tale che il suo campo info non sia x, vLista(L) = Lista(L))
void elimAll2(nodo * L, int x) { //modificato direttamente L: termina appena trova nodo con info == x
    if (L->next) {
...
//POST=(Lista(L) contiene tutti i nodi di vLista(L) a parte ogni con info = x, che, se esiste, va anche deallocato)

//PRE=(Lista(L) ben formata e VLista(L) = Lista(L))
void elimAll3(nodo *& L, int x) { //modificato direttamente L: termina appena trova nodo con info == x
    if (L) {
...
//POST=(Lista(L) è come vLista(L) a parte ogni nodo con info = x, che, se esiste, va anche deallocato)
