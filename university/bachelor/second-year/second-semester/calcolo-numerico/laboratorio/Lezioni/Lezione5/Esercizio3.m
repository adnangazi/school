...
[zero2, res2, iterates2, flag2] = NewtonMod(f3, f4, x0, 3, toll, itmax, method);
[zero3, res3, iterates3, flag3] = NewtonMod(f5, f6, x0, 5, toll, itmax, method);
...
semilogy(abs(s3(2:end))./abs(s3(1:end-1)).^2);
title('|Sk+1|/|Sk|');
...
semilogy(err2(2:end)./abs(s2).^2);
semilogy(err3(2:end)./abs(s3).^2);
title('Ek/|Sk|');
