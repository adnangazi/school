...
T DList<T>::popBack() {
    if (isEmpty()) {
        throw std::runtime_error("Impossibile fare popBack in una lista vuota!");
    }
    T temp = last->info;
    if (size == 1) {
        delete first;
...
T DList<T>::popFront() {
    if (isEmpty()) {
        throw std::runtime_error("Impossibile fare popFront in una lista vuota!");
    }
    T temp = first->info;
    if (size == 1) {
        delete first;
...
T DList<T>::pop(const uint pos) {
    if (isEmpty()) {
        throw std::runtime_error("Impossibile fare pop in una lista vuota!");
    }
    if (pos >= size - 1) {
        return popBack();
    } else if (pos == 0) {
...
void DList<T>::set(const T & i, const uint pos) {
    if (isEmpty() || pos >= size) {
        throw std::runtime_error("Impossibile fare set in una lista vuota o IndexOutOfBoundError!");
    }
    pop(pos);
    push(i, pos);
}
...
T & DList<T>::operator[](const uint pos) const {
    if (isEmpty() || pos >= size) {
        throw std::runtime_error("Impossibile accedere ad una cella di una lista vuota o IndexOutOfBoundError!");
    }
    if (pos == size - 1) {
        return last->info;
    } else if (pos == 0) {
...
//overload degli operatori esterni alla classe
template<class T>
