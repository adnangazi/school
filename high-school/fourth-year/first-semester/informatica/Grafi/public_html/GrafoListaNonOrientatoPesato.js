...
Grafo.prototype.getGrado = function (vert) {//grado in numero
    assert.preCondizione(vert < this.vertici, "vertici in input non validi!");
    return this.adiacenze[vert].length;
}
...
Grafo.prototype.Dijkstra = function (start, end) {//cammino minimo tra 'start' e 'end'.
    ...
    for (var i = 0; i < cammino.length; i++) {
        ...
        for (var j = 0; j < intorno.length; j++) {
            ...
            if (!this.presente(vertice, visitati) && pesi[cammino.indexOf(adiacenza) > pesi[cammino.indexOf(vertice)] + this.adiacenze[start + i][j].peso]) {
